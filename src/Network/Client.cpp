      ...
      socket_(io_service)
  {
    ...
    resolver_.async_resolve(query,
        std::bind(&Client::handle_resolve, this,
		  ...
		  std::placeholders::_2));//asio::placeholders::iterator));
  }



void Client::handle_resolve(const asio::error_code& err, tcp::resolver::iterator endpoint_iterator)
{
	if (!err)
	{
		...
		socket_.async_connect(endpoint, std::bind(&Client::handle_connect, this, std::placeholders::_1, ++endpoint_iterator));
...
void Client::handle_connect(const asio::error_code& err, tcp::resolver::iterator endpoint_iterator)
{
	if (!err)
	{
